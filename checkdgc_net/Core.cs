...
namespace checkdgc_net
{
    class Core
    {
        ...
        private void OnIncomingCameraImage(object sender, EventArgs e)
        {
            ...
            // ----------------------------------------------------
            // DGC detector
            Task<DgcValidationResult> task = Task.Run<DgcValidationResult>(async () => await dgc.ParseDgc(tI.Image));
            string results_info = "";
